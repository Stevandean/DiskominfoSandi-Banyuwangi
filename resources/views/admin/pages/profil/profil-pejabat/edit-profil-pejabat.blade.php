...
@section('container')
  <div class="md:shadow-md bg-white rounded-lg p-3 pt-4">
    ...
    <form id="form-upload" class="p-5" action="/admin/dokumen" method="post" enctype="multipart/form-data"> 
      @csrf
      <div class="mb-6">
        <x-admin.input input-name="Nama Pejabat" form-name="name" input-value="{{ $profilPejabat->name }}"  />
      </div>
      <div class="mb-6">
        <x-admin.input-upload  type="image" input-name="File Foto Pejabat" info-file-read-only="{{ $profilPejabat->image }}" form-name="image" :is-filled=true :is-error=false>
          hanya file gambar (.jpg, .jpeg, .png, etc ) max 10mb.
        </x-admin.input-upload>
      </div>
      <div class="mb-6">
        <x-admin.input-upload type="pdf" input-name="File Profil Pejabat" info-file-read-only="{{ $profilPejabat->profile_pdf }}" form-name="profile_pdf" :is-filled=true :is-error=false>
          hanya file pdf, max 20mb
        </x-admin.input-upload>
      </div>
      <div class="mb-6">
        <x-admin.input-upload type="pdf" input-name="Dokumen lkhpn" info-file-read-only="{{ $profilPejabat->source_lkhpn }}" form-name="source_lkhpn" :is-filled=true :is-error=false>
          hanya file pdf, max 20mb 
        </x-admin.input-upload>
      </div>
      <div class="mb-6">
...
@push('upload-script') 
  <script>
    ...
    function upload(){
      ...
      fetch('/admin/profil/profil-pejabat/{{ $profilPejabat->id }}', {
        method: 'POST',
        headers:{
